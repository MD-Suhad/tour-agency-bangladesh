...
const MyOrders = () => {
  ...
  const url = `https://eerie-mummy-09164.herokuapp.com/user/${email}`;
  useEffect(() => {
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
  ...
  const deleteOrder = (id) => {
    ...
    if (confirmation) {
      const url = `https://eerie-mummy-09164.herokuapp.com/myOrders/${id}`;
      fetch(url, {
        method: "DELETE",
      })
        .then((res) => res.json())
  ...
  return (
    <>
    {myOrders.length > 0 ?<div className="text-center my-5">
      {isLoading ? (
    ...
    :
    <h2 className='text-danger text-center my-5'>Sorry No Orders Found!</h2>
    }
    </>
  );
};
